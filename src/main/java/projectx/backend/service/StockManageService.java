...
public class StockManageService {
    ...
    public double updateEarningsRate(StockManage stockManage) {
        ...
        double currentValue = currentPrice * stockManage.getQuantity();
        double earningsRate = ((currentValue - stockManage.getMoney()) / stockManage.getMoney()) * 100;
    ...
    public StockManage sellStock(Long userId, Long stockId, int quantity) {
        ...
        double currentPrice = stockInfo.getCurrentPrice();
        double saleMoney = currentPrice * quantity;

        // 사용자의 현금 증가
        user.setBalance(user.getBalance() + saleMoney);
        userRepository.save(user);
